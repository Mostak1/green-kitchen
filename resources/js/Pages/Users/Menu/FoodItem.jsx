...
const FoodItem = ({ image, name, price, details, category }) => {
    ...
    return (
        <div className="bg-white border border-gray-100 transition transform duration-700 hover:shadow-xl hover:scale-105 p-4 rounded-lg relative">
            <span className="bg-blue-100 border border-blue-500 rounded-full text-primary text-sm poppins px-4 py-1 inline-block mb-4 ">{category}</span>
            <img className="w-64 mx-auto transform transition duration-300 hover:scale-105" src={`https://res.awcbd.org/storage/menu/${image}`} alt="food" />
            <div className="flex flex-col items-center my-3 space-y-2">
                <h1 className="text-gray-900 poppins text-lg">{name}</h1>
                <p className="text-gray-500 poppins text-sm text-center">{details.slice(0, 50)}</p>
